'use strict';
/**
 * Shop POS Referral Agent — live Windows shell.
 * Loads the Railway referral agent portal so every deploy reaches this PC automatically.
 */
const { app, BrowserWindow, Menu } = require('electron');

const BASE = (process.env.SHOP_POS_SYNC_URL || 'https://chisafood.up.railway.app').replace(/\/$/, '');
const APP_URL = `${BASE}/referral-app.html`;
...
function offlineHtml() {
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>Referral Agent</title>
<style>
...
<body><div class="card">
  <h1>Referral Agent</h1>
  <p>This app needs the internet so it can load the latest Referral Agent changes from your shop.</p>
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: 'persist:shoppos-referral'
    }
  });
  ...
  mainWindow.webContents.on('did-fail-load', (_e, code, desc, url, isMain) => {
    if (!isMain) return;
    console.warn('[referral-agent] did-fail-load', code, desc, url);
    showOffline();
  });

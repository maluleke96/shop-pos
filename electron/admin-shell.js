'use strict';
/**
 * Shop POS Admin — live Windows shell.
 * Loads the Railway admin site so every deploy reaches this PC automatically.
 */
const { app, BrowserWindow, Menu } = require('electron');

const BASE = (process.env.SHOP_POS_SYNC_URL || 'https://chisafood.up.railway.app').replace(/\/$/, '');
const APP_URL = `${BASE}/?app=admin`;
...
function offlineHtml() {
  return `<!doctype html>
<html><head><meta charset="utf-8"><title>Shop POS Admin</title>
<style>
  ...
  button { margin-top:12px; background:#16a34a; color:#fff; border:0; border-radius:10px; padding:10px 16px; font-weight:700; cursor:pointer; }
</style></head>
<body><div class="card">
  <h1>Shop POS Admin</h1>
  <p>This app needs the internet so it can load the latest admin changes from your shop.</p>
...
function createWindow() {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: 'persist:shoppos-admin'
    }
  });

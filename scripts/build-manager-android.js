/**
 * Build Business Manager Android APK (loads cloud /manager/ UI).
 * Output: Downloads/ShopPOS-Installers/Android/ShopPOS-Manager.apk
 */
const fs = require('fs');
...
const cloudUrl = (process.env.SHOP_POS_CLOUD_URL || 'https://peaceful-motivation-production-7dd2.up.railway.app').replace(/\/$/, '');
...
<meta http-equiv="refresh" content="0;url=${cloudUrl}/manager/"></head>
<body><p>Loading Business Manager…</p></body></html>`, 'utf8');
}

function patchStringsXml() {
  const p = path.join(root, 'android', 'app', 'src', 'main', 'res', 'values', 'strings.xml');
  const xml = `<?xml version='1.0' encoding='utf-8'?>\n<resources>
    <string name="app_name">Business Manager</string>
    <string name="title_activity_main">Business Manager</string>
    <string name="package_name">com.shoppos.manager</string>
    <string name="custom_url_scheme">com.shoppos.manager</string>
</resources>\n`;
...
function patchBuildGradle() {
  ...
  g = g.replace(/applicationId\s+"[^"]+"/, 'applicationId "com.shoppos.manager"');
...
const cfg = JSON.parse(fs.readFileSync(path.join(root, 'capacitor-apps', 'capacitor.manager.json'), 'utf8'));
cfg.server = { url: `${cloudUrl}/manager/`, cleartext: false };
...
const dest = path.join(destRoot, 'ShopPOS-Manager.apk');

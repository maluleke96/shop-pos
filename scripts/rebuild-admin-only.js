...
fs.writeFileSync(path.join(root, 'electron-builder.cloud-admin.json'), JSON.stringify(cfg, null, 2));

const eb = require.resolve('electron-builder/cli.js');
const r = spawnSync(process.execPath, [eb, '--config', 'electron-builder.cloud-admin.json', '--win', '--x64'], {
  cwd: root,
  stdio: 'inherit',
...
const dest = path.join(process.env.USERPROFILE || '', 'Downloads', 'ShopPOS-Installers', 'Admin');
fs.mkdirSync(dest, { recursive: true });
for (const f of [`${artifact}-Setup.exe`, `${artifact}-Portable.exe`]) {
  const src = path.join(root, 'dist', 'cloud-apps', 'admin', f);

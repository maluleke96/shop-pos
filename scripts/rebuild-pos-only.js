/** One-off: rebuild POS only into Downloads/ShopPOS-Installers/POS */
const fs = require('fs');
...
const cfg = {
  ...
  extraMetadata: { main: 'electron/cloud-shell-config-pos.js', name: appId },
  win: {
    target: [
...
fs.writeFileSync(path.join(root, 'electron-builder.cloud-pos.json'), JSON.stringify(cfg, null, 2));

const eb = require.resolve('electron-builder/cli.js');
const r = spawnSync(process.execPath, [eb, '--config', 'electron-builder.cloud-pos.json', '--win', '--x64'], {
  cwd: root,
  stdio: 'inherit',
...
const dest = path.join(process.env.USERPROFILE || '', 'Downloads', 'ShopPOS-Installers', 'POS');

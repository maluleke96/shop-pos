/** One-off: rebuild Marketing Agent into Downloads/ShopPOS-Installers/Marketing-Agent */
const fs = require('fs');
...
const cfg = {
  appId,
  productName: name,
  directories: { output: path.join('dist', 'cloud-apps', 'marketing'), buildResources: 'build' },
  files: ['electron/**/*', 'src/**/*', 'mobile/**/*', 'lib/**/*', 'package.json'],
  extraMetadata: { main: 'electron/cloud-shell-config-marketing.js', name: appId },
  win: {
    target: [
...
fs.writeFileSync(path.join(root, 'electron-builder.cloud-marketing.json'), JSON.stringify(cfg, null, 2));

const eb = require.resolve('electron-builder/cli.js');
const r = spawnSync(process.execPath, [eb, '--config', 'electron-builder.cloud-marketing.json', '--win', '--x64'], {
  cwd: root,
  stdio: 'inherit',
...
const dest = path.join(process.env.USERPROFILE || '', 'Downloads', 'ShopPOS-Installers', 'Marketing-Agent');
fs.mkdirSync(dest, { recursive: true });
for (const f of [`${artifact}-Setup.exe`, `${artifact}-Portable.exe`]) {
  const src = path.join(root, 'dist', 'cloud-apps', 'marketing', f);

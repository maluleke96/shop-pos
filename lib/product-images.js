...
function getProductImage(productId) {
  ...
  let row = null;
  try {
    row = db.prepare('SELECT picture_path FROM products WHERE id = ?').get(id);
  } catch (_) {
    try { row = db.prepare('SELECT * FROM products WHERE id = ?').get(id); } catch (__) { /* ignore */ }
  }
...
function publicProductImageUrl(productId, isCombo = false, stamp) {
  if (!productId) return null;
  const base = isCombo ? `/api/combo-image/${productId}` : `/api/product-image/${productId}`;
  if (!stamp) return base;
  return `${base}?v=${encodeURIComponent(String(stamp).slice(0, 19))}`;
}

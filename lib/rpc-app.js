...
async function handleRpcPost({ handlers, session, persistNow, sessions, body, sessionTokenHeader }) {
  ...
  // Phase 5: suspension gate — block customer RPCs when THIS shop record is SUSPENDED
  try {
    const ns = String(method || '').split(':')[0];
    if (ns !== 'platform' && ns !== 'entitlements') {
      const shops = require('../electron/services/platform-shops');
      shops.assertShopNotSuspended();
    }
  } catch (err) {
    if (err && err.code === 'SHOP_SUSPENDED') {
      return {
        status: err.status || 403,
        json: {
          success: false,
        ...
        headers: { 'X-Shop-Status': 'suspended' }
      };
    }
  }

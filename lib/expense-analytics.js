...
function groupByPaymentMethod(rows) {
  const map = {};
  for (const e of rows || []) {
    const m = String(e.payment_method || 'cash').trim().toLowerCase() || 'cash';
...
function groupByVendor(rows) {
  const map = {};
  for (const e of rows || []) {
    let name = String(e.vendor_name || '').trim();
    if (!name) {
      const desc = String(e.description || '').trim();
      name = desc.split(/[·|,–—]/)[0].trim().slice(0, 60) || 'Unknown vendor';
    }
    const key = name.toLowerCase();
    if (!map[key]) map[key] = { name, amount: 0, count: 0 };
    map[key].amount = roundMoney(map[key].amount + (Number(e.amount) || 0));
    map[key].count += 1;
  }
  return Object.values(map).sort((a, b) => b.amount - a.amount);
}

function detectSpendSpikes(rows, asOf = new Date()) {
  const weekStart = toIso(startOfWeek(asOf));
  const weekEnd = toIso(endOfWeek(asOf));
  const lookbackStart = toIso(addDays(parseDate(weekStart), -28));
  const daily = dailyTotals(rows, lookbackStart, weekEnd);
  const history = daily.filter((d) => d.date < weekStart && d.total > 0);
  const avg = history.length
    ? history.reduce((s, d) => s + d.total, 0) / history.length
    : 0;
  const spikes = daily
    .filter((d) => d.date >= weekStart && avg > 0 && d.total >= avg * 2.5)
    .map((d) => ({
      date: d.date,
      amount: d.total,
      avgDaily: roundMoney(avg),
      multiple: avg > 0 ? roundMoney(d.total / avg) : 0,
      message: `${d.date}: spent ${roundMoney(d.total)} — about ${roundMoney(d.total / avg)}× your recent daily average (${roundMoney(avg)}).`
    }));
  return { avgDaily: roundMoney(avg), spikes };
}

function groupByFunding(rows) {
  const map = { business: 0, owner: 0 };
  for (const e of rows || []) {
    const f = String(e.funding_source || 'business').toLowerCase() === 'owner' ? 'owner' : 'business';
    map[f] = roundMoney(map[f] + (Number(e.amount) || 0));
  }
  return map;
}
 ...
 * @param {Date} [asOf]
 * @param {{ revenueWeek?: number, revenueMonth?: number }} [opts]
 */
function computeExpenseDashboard(expenses, branchNames = {}, asOf = new Date(), opts = {}) {
  ...
  const paymentMethodsMonth = groupByPaymentMethod(monthRows);
  const vendorsMonth = groupByVendor(monthRows);
  const spikeInfo = detectSpendSpikes(expenses, asOf);
  const fundingMonth = groupByFunding(monthRows);
  const revW = Number(opts.revenueWeek) || 0;
  const revM = Number(opts.revenueMonth) || 0;
  ...
  return {
    ...
    byPaymentMethod: paymentMethodsMonth.map((p) => ({
      ...p,
      label: p.name,
      pct: monthTotal > 0 ? roundMoney((p.amount / monthTotal) * 100) : 0
    })),
    topVendors: vendorsMonth.slice(0, 15).map((v) => ({
      ...v,
      pct: monthTotal > 0 ? roundMoney((v.amount / monthTotal) * 100) : 0
    })),
    spikes: spikeInfo,
    funding: fundingMonth,
    profitImpact: {
      week: {
        revenue: revW,
        expenses: weekTotal,
        expensePctOfRevenue: revW > 0 ? roundMoney((weekTotal / revW) * 100) : null,
        netAfterExpenses: roundMoney(revW - weekTotal)
      },
      month: {
        revenue: revM,
        expenses: monthTotal,
        expensePctOfRevenue: revM > 0 ? roundMoney((monthTotal / revM) * 100) : null,
        netAfterExpenses: roundMoney(revM - monthTotal)
      }
    },
    charts: {
      ...
      payments: paymentMethodsMonth.slice(0, 8)
    },
    advice,
    expenseCount: {

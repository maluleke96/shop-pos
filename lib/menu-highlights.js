...
function parseSettings(settingsParsed) {
  ...
  const mode = raw.new_arrival_mode === 'date' ? 'date' : 'days';
  return {
    new_arrival_days: Number(raw.new_arrival_days) || DEFAULTS.new_arrival_days,
    new_arrival_mode: mode,
    new_arrival_until: raw.new_arrival_until || null,
...
function resolveNewArrivalUntil(settingsParsed, overrideDate) {
  if (overrideDate) return String(overrideDate).slice(0, 10);
  const cfg = parseSettings(settingsParsed);
  if (cfg.new_arrival_mode === 'date' && cfg.new_arrival_until) {
    return String(cfg.new_arrival_until).slice(0, 10);
  }
  const until = new Date();
  until.setDate(until.getDate() + (cfg.new_arrival_days || DEFAULTS.new_arrival_days));
...
function markNewArrival(db, productId, settingsParsed, untilOverride) {
  const untilStr = resolveNewArrivalUntil(settingsParsed, untilOverride);
  try {
